...
odoo.define('app_web_one2many_multi_add.form_relational_extend_v11', function (require) {
    ...
    //    }),
    // });
    var MultiFieldOne2Many = FieldOne2Many.extend({
     ...
     */
        _onAddMultiRecord: function (ev, from_multi) {
            ...
            // var paternal_field = this.attrs.paternal_field;
            ...
            var dp = ancestor.model._makeDefaultRecord(params.model, {
                ...
                // 增加前端的在库不在库筛选 rock
                if ( self.record.data.ana_picking_type_id ) {
                    if (self.record.data.ana_picking_type_id.data.display_name == '取') {
                    domain = domain.concat([['state','in',['在库']]])
                    } else {
                    domain = domain.concat([['state','in',['不在库', '草稿']]]);
                    };}
                ...
                // self.record.data.ana_picking_type_id.data.display_name
                new dialogs.SelectCreateDialog(self, {
                    res_model: res_model,
                    domain: domain.concat(["!", ["id", "in", self.value.data.map(function(i){return i.data[res_field].data.id;})]]),
                    // 用于拼接 domain 选择项 rock 
                    context: context,
                    ...
                    on_selected: function (records) {
                        _.each(records, function (record) {
                            ...
                            // self.trigger_up('quick_create', {id: self.value.id });
                            self._setValue({
                                operation: 'CREATE',
    ...
    registry.add('multi_select_tree', MultiFieldOne2Many)
